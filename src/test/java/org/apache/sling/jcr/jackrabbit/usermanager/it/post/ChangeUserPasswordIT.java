...
public class ChangeUserPasswordIT extends UserManagerTestSupport {
    ...
    public void changePasswordAsSelfGranted() throws IOException, RepositoryException {
        ...
        try {
            ...
            Dictionary<String, Object> newServiceProps =
                    replaceConfigProp(originalServiceProps, "allowSelfChangePassword", Boolean.TRUE);
            configuration.update(newServiceProps);
            new WaitForServiceUpdated(
                    5000, 100, bundleContext, ChangeUserPassword.class, "allowSelfChangePassword", Boolean.TRUE);
            ...
            changeUserPassword.changePassword(
                    user1Session, user1.getID(), "testPwd", "testPwdChanged", "testPwdChanged", new ArrayList<>());
            try {
                user1Session.save();
            } catch (AccessDeniedException e) {
                logger.error(
                        String.format(
                                "Did not expect AccessDeniedException when changing user passsword: %s",
                                e.getMessage()),
                        e);
    ...
    public void changePasswordAsSelfDenied() throws IOException, RepositoryException {
        ...
        try {
            ...
            Dictionary<String, Object> newServiceProps =
                    replaceConfigProp(originalServiceProps, "allowSelfChangePassword", Boolean.FALSE);
            configuration.update(newServiceProps);
            new WaitForServiceUpdated(
                    5000, 100, bundleContext, ChangeUserPassword.class, "allowSelfChangePassword", Boolean.FALSE);
    ...
    public void changePasswordAsSelfWithoutOldPasswordFails() throws IOException, RepositoryException {
        ...
        try {
            ...
            @NotNull String user1Id = user1.getID();
            try {
                changeUserPassword.changePassword(
                        user1Session, user1Id, null, "testPwdChanged", "testPwdChanged", new ArrayList<>());
            ...
            try {
                changeUserPassword.changePassword(
                        user1Session, user1Id, "", "testPwdChanged2", "testPwdChanged2", new ArrayList<>());
                fail("Expected a RepositoryException when changing user passsword.");
            } catch (RepositoryException e) {
                assertEquals("Old Password was not submitted", e.getMessage());
                user1Session.refresh(false);
    ...
    public void changePasswordAsUserAdminMemberWithoutOldPassword() throws RepositoryException {
        ...
        try {
            ...
            Group userAdminGroup = createGroup.createGroup(
                    adminSession,
                    userAdminGroupName,
                    Collections.singletonMap(":member", user1.getID()),
                    new ArrayList<>());
            if (adminSession.hasPendingChanges()) {
                adminSession.save();
            }
            ...
            try {
                changeUserPassword.changePassword(
                        user1Session, user2.getID(), null, "testPwdChanged", "testPwdChanged", new ArrayList<>());
            } catch (RepositoryException e) {
            ...
            try {
                changeUserPassword.changePassword(
                        user1Session, user2.getID(), "", "testPwdChanged2", "testPwdChanged2", new ArrayList<>());
            } catch (RepositoryException e) {
                fail("Did not expect a RepositoryException when changing user passsword.");
            }
            try {
    ...
    public void changePasswordAsSelfGrantedForOtherUserWithOldPassword() throws RepositoryException, IOException {
        ...
        try {
            ...
            try {
                changeUserPassword.changePassword(
                        user1Session, user2.getID(), "testPwd", "testPwdChanged", "testPwdChanged", new ArrayList<>());
            } catch (RepositoryException e) {
                fail("Did not expect a RepositoryException when changing user passsword.");
            }
            try {
